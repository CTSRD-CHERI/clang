...
// RUN: %clang_cc1 -triple cheri-unknown-freebsd -DWITH_CHERI_CASTS %s -emit-llvm -o - -O2 | FileCheck %s -check-prefixes BOTH,HYBRID -enable-var-scope
// RUN: %clang_cc1 -triple cheri-unknown-freebsd -DWITH_CHERI_CASTS -target-abi purecap %s -emit-llvm -o - -O2 | FileCheck %s -check-prefixes BOTH,PURECAP -enable-var-scope
